...
namespace RebuildFileLists
{
    internal class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            if (Directory.Exists(Path.Combine(installPath, "archives_win32")) == true)
            {
                inputPaths.AddRange(Directory.GetFiles(Path.Combine(installPath, "archives_win32"), "pc*.tab", SearchOption.AllDirectories));
                inputPaths.AddRange(Directory.GetFiles(Path.Combine(installPath, "archives_win32"), "game*.tab", SearchOption.AllDirectories));
            }

            if (Directory.Exists(Path.Combine(installPath, "archives_win32_ext")) == true)
            {
                inputPaths.AddRange(Directory.GetFiles(Path.Combine(installPath, "archives_win32_ext"), "*.tab", SearchOption.AllDirectories));
            }

            if (Directory.Exists(Path.Combine(installPath, "dlc")) == true)
            {
                inputPaths.AddRange(Directory.GetFiles(Path.Combine(installPath, "dlc"), "pc*.tab", SearchOption.AllDirectories));
            }
            ...
            foreach (var inputPath in inputPaths)
            {
                ...
                foreach (var nameHash in tab.Keys.Select(e => e).Distinct())
                {
